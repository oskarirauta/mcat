...
- **Browser mode** (pager): scroll with arrow keys / PgUp / PgDn, `Space`
  ...
  `/` searches (`n`/`N` for next/previous, matches shown in reverse video),
  `g`/`G` jump to top/bottom, `q` or `Esc` quits. A status line shows
  `line N/total` and the position (`TOP`/`END`/`%`).

Colours follow `--color=auto|always|never` (default `auto`). In `auto` mode
colours are on when stdout is a terminal and off when piped; the `NO_COLOR`
environment variable disables them. `--color=always` forces colours (handy for
`mcat file | less -R` or as a git pager/difftool).
...
-C, --no-color      disable colours (same as --color=never)
-c, --color <when>  when to colour: auto|always|never (default auto)
